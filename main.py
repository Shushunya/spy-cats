...
class CatBase(SQLModel):
    name: str 
    years_of_exp: int | None = Field(default=0, ge=0, le=20, description="Years of experience in cat-related activities")
    breed: str = Field(
...
class TargetBase(SQLModel):
    ...
    country: str
    notes: str
    complete_state: bool = Field(default=False, description="Indicates if the target is complete or not")
...
class MissionBase(SQLModel):
    cat_id: int | None = Field(default=None, foreign_key="cat.id", description="ID of the cat assigned to the mission")
    target_id: int | None = Field(default=None, foreign_key="target.id", description="ID of the target for the mission")
    complete_state: bool = Field(default=False, description="Indicates if the mission is complete or not")
...
async def update_cat_salary(cat_id: int, session: SessionDep, new_salary: float = Query(ge=0)):
...
async def delete_mission(mission_id: int, session: SessionDep):
    ...
    if not mission:
        raise HTTPException(status_code=404, detail="Mission not found")
    if mission.cat_id:
        raise HTTPException(status_code=400, detail="Cannot delete mission with assigned cat")
...
async def update_mission_target(
    ...
    if notes is not None:
        if db_target.complete_state or db_mission.complete_state:
            raise HTTPException(status_code=400, detail="Cannot update notes for a completed mission")
